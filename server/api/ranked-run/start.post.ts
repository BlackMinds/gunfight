...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const progress = await database().query<{ highest_stage: number }>('SELECT highest_stage FROM gunfight_ranked_progress WHERE user_id = $1', [user.sub])
    ...
    await database().query(`
      WITH expired AS (
        UPDATE gunfight_ranked_runs SET status = 'expired'
