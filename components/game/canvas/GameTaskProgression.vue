<template>
  <section class="progression-section">
    ...
    <div class="pity-status">
      <small>保底：Boss {{ dropPity.bossKills }} · 史诗计数 {{ dropPity.epicMisses }}/100 · 传说计数 {{ dropPity.legendaryMisses }}/30 · 神话碎片 {{ dropPity.mythicShards }}/10</small>
      <button type="button" data-testid="redeem-mythic" :disabled="!canRedeemMythicShards" @click="redeemMythicShards">兑换神话配件</button>
    </div>
  </section>
</template>
...
const { dailyTasks, weeklyTasks, achievements, completedDailyTasks, completedWeeklyTasks, completedAchievements, claimTask, dropPity, canRedeemMythicShards, redeemMythicShards } = useGameCanvasContext()
