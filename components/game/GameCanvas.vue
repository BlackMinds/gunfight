<template>
  <main class="game-screen">
    ...
    <aside class="hud hud-right">
      ...
      <h2>{{ weapon.name }} · {{ weapon.rarity }}</h2>
    ...
    <section class="skill-bar" aria-label="战术技能">
      <button v-for="skill in skills" :key="skill.key" type="button" :disabled="skill.cooldown > 0" @click="useSkill(skill.key)">
    ...
    <section v-if="upgradeChoices.length" class="choice-panel">
    ...
    <section v-if="message" class="result">
      <strong>{{ message.title }}</strong>
      <span>{{ message.body }}</span>
      <div class="reward-grid" v-if="message.reward">
        <span>金币 +{{ message.reward.gold }}</span>
        <span>经验 +{{ message.reward.exp }}</span>
        <span>合金 +{{ message.reward.alloy }}</span>
        <span>零件 +{{ message.reward.parts }}</span>
      </div>
      <button type="button" @click="restartStage">{{ message.action }}</button>
    </section>
  </main>
</template>
...
type Message = { title: string; body: string; action: string; advance: boolean; reward?: Reward }
...
const resources = reactive({ gold: 0, alloy: 0, parts: 0 })
...
function spawnEnemy(forceBoss = false) {
  ...
  const area = getPlayArea(width, height)
  const edge = Math.floor(Math.random() * 4)
  const spawnInset = forceBoss ? 28 : 18
  const x =
    edge === 1
      ? area.x + area.width - spawnInset
      : edge === 3
        ? area.x + spawnInset
  ...
  enemies.push({
    ...
    hp: stats.hp * (boss ? 9 : elite ? 1.9 : 1),
    maxHp: stats.hp * (boss ? 9 : elite ? 1.9 : 1),
    speed: stats.speed * (boss ? 0.68 : elite ? 1.08 : 1),
    damage: stats.damage * (boss ? 1.7 : 1),
    radius: boss ? 34 : elite ? 18 : 13,
    elite,
    boss,
    kind,
    label: boss ? '街区首领' : elite ? `精英${stats.label}` : stats.label
  })
}
...
function maybeOfferUpgrade() {
  ...
  upgradeChoices.value = [
    ...
    { name: '火力压制', desc: '伤害提高 18%', apply: () => { modifiers.damage *= 1.18 } },
...
function drawPlayArea(ctx: CanvasRenderingContext2D, area: PlayArea, width: number, height: number) {
  ...
  ctx.lineWidth = 3
  ctx.beginPath()
  ctx.moveTo(area.x, area.y + corner)
  ctx.lineTo(area.x, area.y)
  ctx.lineTo(area.x + corner, area.y)
  ctx.moveTo(area.x + area.width - corner, area.y)
  ctx.lineTo(area.x + area.width, area.y)
  ctx.lineTo(area.x + area.width, area.y + corner)
  ctx.moveTo(area.x + area.width, area.y + area.height - corner)
  ctx.lineTo(area.x + area.width, area.y + area.height)
  ctx.lineTo(area.x + area.width - corner, area.y + area.height)
  ctx.moveTo(area.x + corner, area.y + area.height)
  ctx.lineTo(area.x, area.y + area.height)
  ctx.lineTo(area.x, area.y + area.height - corner)
  ctx.stroke()
  ctx.restore()
}
...
function useSkill(key: SkillKey) {
  ...
  if (!skill || skill.cooldown > 0 || message.value || upgradeChoices.value.length) return
  if (key === 'dash') {
    dashTimer = 0.55
    player.invuln = Math.max(player.invuln, 0.55)
    const input = inputVector()
    const hasInput = Math.hypot(input.x, input.y) > 0.1
    player.vx += (hasInput ? input.x : Math.cos(player.angle)) * 520
    player.vy += (hasInput ? input.y : Math.sin(player.angle)) * 520
    skill.cooldown = 6
  }
  if (key === 'overload') {
    overloadTimer = 4
...
function update(delta: number) {
  if (message.value || upgradeChoices.value.length) return
  const canvas = canvasRef.value
  if (!canvas) return
  const width = canvas.clientWidth
  const height = canvas.clientHeight
  ...
  const speedBoost = dashTimer > 0 ? 2.2 : 1
  const targetSpeed = player.speed * modifiers.speed * speedBoost
  const targetVx = movement.x * targetSpeed
  const targetVy = movement.y * targetSpeed
  const acceleration = dashTimer > 0 ? 15 : 8.5
  const drag = moving ? acceleration : 10.5
  player.vx = lerp(player.vx, targetVx, drag * delta)
  player.vy = lerp(player.vy, targetVy, drag * delta)
  const playerMinX = area.x + player.radius
  const playerMaxX = area.x + area.width - player.radius
  const playerMinY = area.y + player.radius
  const playerMaxY = area.y + area.height - player.radius
  player.x = clamp(player.x + player.vx * delta, playerMinX, playerMaxX)
  player.y = clamp(player.y + player.vy * delta, playerMinY, playerMaxY)
  if (player.x <= playerMinX || player.x >= playerMaxX) player.vx *= -0.18
  if (player.y <= playerMinY || player.y >= playerMaxY) player.vy *= -0.18
  ...
  const desiredAngle = aimTarget
    ? Math.atan2(aimTarget.y - player.y, aimTarget.x - player.x)
    : moving
      ? Math.atan2(player.vy, player.vx)
      : player.angle
  player.angle = lerpAngle(player.angle, desiredAngle, (aimTarget ? 12 : 8) * delta)
  ...
  if ((dashTimer > 0 || Math.hypot(player.vx, player.vy) > 120) && player.afterimageTimer <= 0) {
    afterimages.push({
      ...
      y: player.visualY,
      angle: player.angle,
      life: dashTimer > 0 ? 0.26 : 0.16,
      maxLife: dashTimer > 0 ? 0.26 : 0.16,
      size: player.radius * (dashTimer > 0 ? 3.7 : 3.25)
    })
  ...
  const spawnInterval = Math.max(0.28, 1.0 - stage.value * 0.01)
  if (spawnTimer <= 0) {
  ...
  const activeFireRate = weapon.fireRate * modifiers.fireRate * (overloadTimer > 0 ? 1.75 : 1)
  if (player.fireTimer <= 0) {
    shootNearest()
    player.fireTimer = 1 / activeFireRate
  }

  for (const enemy of enemies) {
    enemy.wobble += delta * (enemy.kind === 'fast' ? 8 : 4)
    const angle = Math.atan2(player.y - enemy.y, player.x - enemy.x)
    const side = Math.sin(enemy.wobble) * (enemy.kind === 'fast' ? 0.55 : 0.2)
    const desiredAngle = angle + side
    const targetVx = Math.cos(desiredAngle) * enemy.speed
    const targetVy = Math.sin(desiredAngle) * enemy.speed
    enemy.vx = lerp(enemy.vx, targetVx, (enemy.kind === 'heavy' || enemy.boss ? 3.2 : 6.5) * delta)
    enemy.vy = lerp(enemy.vy, targetVy, (enemy.kind === 'heavy' || enemy.boss ? 3.2 : 6.5) * delta)
    const enemyMinX = area.x + enemy.radius
    const enemyMaxX = area.x + area.width - enemy.radius
    const enemyMinY = area.y + enemy.radius
    const enemyMaxY = area.y + area.height - enemy.radius
    enemy.x = clamp(enemy.x + enemy.vx * delta, enemyMinX, enemyMaxX)
    enemy.y = clamp(enemy.y + enemy.vy * delta, enemyMinY, enemyMaxY)
    if (enemy.x <= enemyMinX || enemy.x >= enemyMaxX) enemy.vx *= -0.2
    if (enemy.y <= enemyMinY || enemy.y >= enemyMaxY) enemy.vy *= -0.2
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (
      bullet.life <= 0 ||
      bullet.x < area.x - margin ||
      bullet.y < area.y - margin ||
      bullet.x > area.x + area.width + margin ||
      bullet.y > area.y + area.height + margin
    ) bullets.splice(i, 1)
  }
  ...
  if (player.hp <= 0) {
    message.value = { title: '撤离失败', body: '保留部分金币，强化后再冲。', action: '重新挑战', advance: false }
  }
  ...
  if (kills.value >= targetKills.value && bossCleared) {
    ...
    message.value = {
      ...
      body: `${stageMeta.value.name} 已肃清，下一关敌人会更密集。`,
      action: '进入下一关',
      advance: true,
      reward
    }
...
function draw() {
  ...
  const cameraX = clamp((player.visualX - width / 2) * -0.018, -10, 10)
  const cameraY = clamp((player.visualY - height / 2) * -0.018, -10, 10)
  const shake = dashTimer > 0 ? Math.sin(stageTimer * 80) * 2 : 0
  ...
  for (const drop of drops) {
    const bob = Math.sin(stageTimer * 5 + drop.x * 0.01) * 2
    drawShadow(ctx, drop.x, drop.y + 7, drop.radius * 3, 0.18)
    drawSprite(ctx, drop.kind === 'gold' ? sprites.pickupGold : sprites.pickupExp, drop.x, drop.y + bob, drop.radius * 4)
  }

  for (const bullet of bullets) {
    drawSprite(ctx, sprites.bullet, bullet.x, bullet.y, overloadTimer > 0 ? 34 : 26, Math.atan2(bullet.vy, bullet.vx))
  }
  ...
  for (const enemy of enemies) {
    ...
    const lean = clamp(enemySpeed / 340, 0, 0.16) * Math.sin(enemy.wobble)
    const size = enemy.radius * (enemy.boss ? 3.6 : 3.2) * (1 + Math.sin(enemy.wobble * 2) * 0.018)
    drawShadow(ctx, enemy.x, enemy.y, size, enemy.boss ? 0.45 : 0.3)
    drawSprite(ctx, enemySprite(enemy), enemy.x, enemy.y + Math.sin(enemy.wobble * 2) * 1.4, size, enemy.angle + lean)
  ...
  for (const ghost of afterimages) {
    drawSpriteTint(
      ...
      ghost.size,
      ghost.angle,
      (ghost.life / ghost.maxLife) * 0.32,
      dashTimer > 0 ? '#e5b84b' : '#6ea6c9'
    )
  }

  const playerSize = player.radius * 3.4 * (1 + clamp(speed / 650, 0, 0.08))
  const bob = Math.sin(player.bob) * clamp(speed / 260, 0, 2.3)
  const lean = clamp(speed / 520, 0, 0.18) * Math.sin(player.bob * 0.5)
  drawShadow(ctx, player.visualX, player.visualY, playerSize, 0.36)
  drawSprite(ctx, sprites.player, player.visualX, player.visualY + bob, playerSize, player.angle + lean)
  if (dashTimer > 0) {
    ctx.save()
    ctx.translate(player.visualX, player.visualY)
    ctx.strokeStyle = 'rgba(229, 184, 75, 0.65)'
    ctx.lineWidth = 3
    ctx.beginPath()
    ctx.arc(0, 0, 28 + (0.55 - dashTimer) * 55, 0, Math.PI * 2)
    ctx.stroke()
    ctx.strokeStyle = 'rgba(229, 184, 75, 0.25)'
    ctx.beginPath()
    ctx.moveTo(-player.vx * 0.11, -player.vy * 0.11)
    ctx.lineTo(0, 0)
    ctx.stroke()
    ctx.restore()
  }
...
function restartStage() {
  if (message.value?.advance) stage.value += 1
  message.value = null
  kills.value = 0
  stageTimer = 0
  bossSpawned = false
