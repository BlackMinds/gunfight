...
describe('完整内容扩展成长状态', () => {
  it('钳制旧存档中的资源、技能、商店和赛季数据', () => {
    ...
    expect(normalizeShopState()).toEqual({ stock: { 'precision-kit': 3, 'reforge-cache': 2, 'attachment-crate': 1, 'weapon-crate': 2 } })
    expect(normalizeShopState({ stock: { 'precision-kit': 99, 'reforge-cache': -2, 'attachment-crate': 1, 'weapon-crate': 1 } })).toEqual({ stock: { 'precision-kit': 3, 'reforge-cache': 0, 'attachment-crate': 1, 'weapon-crate': 1 } })
    expect(normalizeSeasonState({ score: 601 }).score).toBe(601)
    expect(normalizeSeasonState({ id: 'S01-联合作战', score: 601, eventClears: 3, bestBountySeconds: 22, bestSurvivalKills: 88 }, 'S02-联合作战')).toEqual({
      id: 'S02-联合作战', score: 0, eventClears: 0, bestBountySeconds: null, bestSurvivalKills: 0
    })
