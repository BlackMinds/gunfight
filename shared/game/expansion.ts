...
export type ShopOfferId = 'precision-kit' | 'reforge-cache' | 'attachment-crate' | 'weapon-crate'
...
export const shopOffers = [
  ...
  { id: 'weapon-crate', label: '战区武器箱', detail: '获得已解锁武器核心并重铸词条', cost: { gold: 900, honor: 25 }, grant: { precision: 0, reforgeChips: 0 } }
] as const
export const emptyShopState = (): ShopState => ({ stock: { 'precision-kit': 3, 'reforge-cache': 2, 'attachment-crate': 1, 'weapon-crate': 2 } })
...
export const LOCAL_SEASON_ID = 'LOCAL-终局前线'
export const emptySeasonState = (id = LOCAL_SEASON_ID): SeasonState => ({ id, score: 0, eventClears: 0, bestBountySeconds: null, bestSurvivalKills: 0 })
export function normalizeSeasonState(saved?: Partial<SeasonState>, expectedId?: string): SeasonState {
  const savedId = typeof saved?.id === 'string' && saved.id ? saved.id : LOCAL_SEASON_ID
  const id = expectedId || savedId
  if (expectedId && savedId !== expectedId) return emptySeasonState(expectedId)
  return { id, score: Math.max(0, Math.floor(Number(saved?.score) || 0)), eventClears: Math.max(0, Math.floor(Number(saved?.eventClears) || 0)), bestBountySeconds: saved?.bestBountySeconds == null ? null : Math.max(0, Number(saved.bestBountySeconds) || 0), bestSurvivalKills: Math.max(0, Math.floor(Number(saved?.bestSurvivalKills) || 0)) }
}
